...
data IntBST = Nil                     -- ← An empty tree
            | Node Int IntBST IntBST  -- ← A node with an Int value and two
            deriving (Eq, Show)       --   children that are also IntBSTs
...
{- ... the next case is inserting into a non-empty tree (i.e. a `Node`). We
 ...
 then our problem has been reduced to inserting a value into an IntBST again.
